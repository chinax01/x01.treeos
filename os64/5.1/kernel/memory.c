...
struct  Slab_cache* slab_create(unsigned long size, void* (*constructor)(void* , unsigned long), 
	void* (*destructor)(void* , unsigned long), unsigned long arg)
{
	...
	slab_cache->cache_mda_pool = NULL;
...
unsigned long slab_init()
{
	...
	for (i=0; i < 16; i++) {
		...
		memory_management_struct.end_of_struct = memory_management_struct.end_of_struct + 
	...
	for (i=0; i<16; i++) {
		virtual = (unsigned long*)(
			(memory_management_struct.end_of_struct + PAGE_2M_SIZE * i + PAGE_2M_SIZE - 1) 
			& PAGE_2M_MASK);
	...
	color_printk(ORANGE,BLACK,
		...
		memory_management_struct.end_data,
		memory_management_struct.end_brk, 
