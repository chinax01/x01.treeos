...
void init_idt_desc(unsigned char vector,
						  unsigned char desc_type, int_handler handler, unsigned char privilege)
{
	struct Gate *g = &idt[vector];
	unsigned int base = (unsigned int)handler;
	g->offset_low = base & 0xFFFF;
	g->selector = SELECTOR_KERNEL_CS;
	g->dcount = 0;
	g->attr = desc_type | (privilege << 5);
...
{
	struct Descriptor *desc = &gdt[seg >> 3];
...
void init_protect()
{
	...
	init_descriptor(&gdt[INDEX_TSS], vir2phys(seg2linear(SELECTOR_KERNEL_DS), &tss),
	...
	{
		memset(&proc_table[i], 0, sizeof(struct Process));
...
void exception_handler(int vec_no, int err_code, int eip, int cs, int eflags)
{
	...
	// }
	// disp_pos = 0;
	disp_color_str("Exception -->", color);
	disp_color_str(msg[vec_no], color);
	disp_color_str("\n\n", color);
	disp_color_str("EFLAGS:", color);
	disp_int(eflags);
	disp_color_str("CS:", color);
	disp_int(cs);
	disp_color_str("EIP:", color);
	disp_int(eip);

	if (err_code != 0xFFFFFFFF)
	{
		disp_color_str("Error code:", color);
		disp_int(err_code);
	}
}
