...
static void hd_rdwt(struct Message* m);
static void hd_ioctl(struct Message* m);
...
static void hd_identify(int drive)
{
	...
	hd_info[drive].primary[0].size = ((int)hdi[61] << 16) + hdi[60];
}
...
static void print_hdinfo(struct hd_info* hdi)
{
	...
	for (i=0; i < NR_PART_PER_DRIVE + 1; i++) {
		printl("%sPART_%d: base %d(0x%x), size %d(0x%x) (in sector)\n",
			i == 0 ? " " : "    ", i,
			hdi->primary[i].base, hdi->primary[i].base,
			hdi->primary[i].size, hdi->primary[i].size);
	}
