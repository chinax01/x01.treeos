...
static int alloc_imap_bit2(int dev)
{
	int inode_nr = 0;
	int i, j, k;
...
static int alloc_smap_bit2(int dev, int nr_sects_to_alloc)
{
	int i, j, k;
	struct super_block *sb = get_super_block(dev);
	int smap_blk0_nr = 1 + 1 + sb->nr_imap_sects;
...
static struct inode* new_inode2(int dev, int inode_nr, int start_sect)
{
...
static void new_dir_entry2(struct inode* dir_inode, int inode_nr, char* filename)
{
	int dir_blk0_nr = dir_inode->i_start_sect;
	int nr_dir_blks = (dir_inode->i_size + SECTOR_SIZE) / SECTOR_SIZE;
