...
int read_head(struct m_inode* inode, int blocks)
{
	...
	for (count = 0; count < blocks; count++) {
		if (!inode->i_zone[count+1]) continue;
...
int read_area(struct m_inode* inode, long size)
{
	...
	for (count=0; count < 512; count++) {
		if ((i = read_ind(inode->i_dev, *(table++), size, BLOCK_SIZE*(518+count)))
			|| (size -= BLOCK_SIZE * 512) <= 0)
		{
			return i;
		}
...
static unsigned long copy_strings(int argc, char** argv, 
	...
	char ** argv, char ** envp)
{
	...
	if ((0xffff & eip[1]) != 0x000f)
		panic("execve calle from supervisor mode");
