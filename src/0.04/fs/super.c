...
#define set_bit(n, addr)	({ \
register int __res; 	\
__asm__("bt %2, %3; setb %%al" : "=a"(__res) : "a"(0), "r"(n), "m"(*(addr))); \
__res; })
...
struct super_block* do_mount(int dev)
{
	...
	for (p = &super_block[0]; p < &super_block[NR_SUPER]; p++) 
		if (!(p->s_dev)) break;
	p->s_dev = -1;
	if (p >= &super_block[NR_SUPER]) return NULL;
	if (!(bh = bread(dev, 1))) return NULL;
...
void mount_root(void) 
{
	...
	if (!(mi = iget(ROOT_DEV, 1)))
		panic("Unalble to read root i-node");
	mi->i_count += 3;
