...
// fsfl: 扫描 eax 首个是1 的位， 把偏移值存入 edx。
// lodsl: 取 [esi] => eax
...
void free_block(int dev, int block)
{
	...
	if (!(sb = get_super(dev)))
		panic("Trying to free block on nenexistent device");
	...
	if (clear_bit(block & 8191, sb->s_zmap[block/8192]->b_data)) {
		...
		panic("free_block: bit already cleare");
	}
...
int new_block(int dev)
{
	...
	if (!(sb = get_super(dev)))
		panic("Trying to get enw block from nonexistent device");
	j = 8192;
	for (i=0; i<8; i++) {
		if (bh = sb->s_zmap[i])
	...
	if (!(bh = getblk(dev, j)))
		panic("new_block: connot get block");
	if (bh->b_count != 1)
